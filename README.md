...
> **Status:** built and tested (434 hermetic tests, `tsc` clean, plus live
...
## The kernel vault (works with zot2vault, never requires it)

The plugin is fully useful on an empty vault. But if the vault was generated by
zot2vault (the companion desktop app that mirrors a Zotero library), arrivals
connect to that existing graph: the plugin reads `.scriptorium/state.json`
(READ-ONLY — zot2vault is that file's only writer; plugin state lives in
`data.json`) to resolve origin ids to existing `Papers/` notes, so citation
edges point at the user's real library and fetched papers the user already owns
are never duplicated.

Flows between the two tools:

- **Inbox → Zotero:** every inbox note carries its DOI/arXiv id and URL; a
  "Copy identifier" command feeds Zotero's *Add by Identifier*. (Later,
  desktop-gated: "Send to Zotero" via Zotero desktop's local connector API on
  127.0.0.1:23119 — never by touching Zotero's database.)
- **Keeping a paper you moved into `Papers/`:** if that paper later enters your
  Zotero library too, zot2vault regenerates the same file and *upgrades* it in
  place — richer metadata, annotations, PDF-derived citations — while
  preserving anything you wrote below the generated-section marker. That works
  because both tools share the same filename rules and the same section
  markers (see `docs/interop-spec.md`); it is not an accident, and breaking
  either would turn the upgrade into a duplicate file.
...
- **Zotero → vault:** that's just re-running zot2vault (cheap, incremental);
  an optional desktop-gated command runs a user-supplied zot2vault executable.

## Non-negotiables (community review + project principles)

- `requestUrl()` for all network access, never `fetch()`; Obsidian Vault API for
  all file access, never Node `fs`. `isDesktopOnly: false` — everything except
...
literature-inbox/
  ...
  src/
    ...
    core/                zero Obsidian imports, therefore unit-testable:
                         ...
                         citations, notes, inbox-page, vault-state,
                         update, prune, hash, xml, types
  tests/
    fixtures/            real recorded API responses + a real state.json
                         generated by zot2vault itself
    *.test.ts            146 tests
```
...
**Verified:** 213 tests green — including 32 that drive the real plugin class
end to end through a faked Obsidian API, and cross-tool interop against a
`state.json` generated by the real zot2vault. Typecheck and build clean.
A further 9 checks run against the live OpenAlex and arXiv APIs
